...
def main(page: ft.Page):
    ...
    # Variables globales pour l'historique de chat et la gestion de session
    messages = []  # Liste pour stocker les messages échangés
    history = {}   # Dictionnaire pour l'historique de chat
    current_chat_id = datetime.now().strftime("%Y%m%d%H%M%S")  # ID unique pour chaque chat
    
    HISTORY_FILE = "chat_history.json"  # Fichier où l'historique est sauvegardé
    
    # Clé API et modèle de Gemini (IA)
    gemini_api_key = "AIzaSyBrSpD_nQe8zU1JHStOYjNPnVr5B48EktY"  # Remplacer par la clé API réelle
    gemini_model = "gemini-1.5-flash"  # Choix du modèle Gemini
    
    # Fonction pour charger l'historique des chats à partir d'un fichier
    def load_history():
        try:
    ...
    # Fonction pour sauvegarder l'historique des chats dans un fichier
    def save_history(history):
        with open(HISTORY_FILE, "w") as file:
            json.dump(history, file, indent=4)
    
    # Chargement de l'historique dès le début
    history = load_history()
    
    # Définition de la structure du chat
    chat_column = ft.Column(
        spacing=16,
        scroll=None, 
        expand=True,  # La colonne occupe tout l'espace disponible
    )
    ...
    chat_list_view = ft.ListView(
        ...
        spacing=0,
        padding=0,
        auto_scroll=True,  # Défilement automatique lorsque de nouveaux messages arrivent
    )
    ...
    # Zone contenant le champ de message et le bouton d'envoi
    ...
    # Formatage de l'heure pour les horodatages des messages
    def format_timestamp():
        now = datetime.now()
        return now.strftime("%I:%M %p")
    
    # Fonction pour copier un message dans le presse-papiers
    ...
    # Fonction pour afficher les messages dans la vue
    def display_messages():
        chat_column.controls.clear()
        for msg in messages:
            is_user = msg["role"] == "user"  # Vérifie si le message est de l'utilisateur
            ...
            if is_user:
                ...
                # Affichage du message de l'assistant (Sofia)
                message_content = ft.Column([
                    ft.Text(
                        label,
                        size=12,
            ...
            # Ajouter chaque message dans la colonne
    ...
    def send_message(e):
        ...
        # Ajouter le message de l'utilisateur dans la vue
        ...
        try:
            # Interaction avec l'API Gemini pour obtenir la réponse
        ...
        # Sauvegarde de l'historique après chaque chat
        history[current_chat_id] = messages
        save_history(history)
        
        # Réinitialisation de la zone d'entrée
        user_message.value = ""
    ...
    def show_history(e):
        ...
        # Affichage de l'historique des chats
        ...
        # Affichage de l'historique sur la page
        page.add(
            ft.Column([
                app_bar,
                ft.Container(
    ...
    # Fonction pour charger l'interface principale
    def load_main_interface():
        ...
        # Ajouter l'interface principale à la page

        page.add(main_column)
        display_messages()  # ajout des messages sur l'ecran
        # Vérifier s'il y a des messages dans l'historique, si oui, faire défiler la fenêtre de chat jusqu'à la fin
        if len(messages) > 0:
            chat_list_view.scroll_to(offset=float('inf'), duration=300)  # Fait défiler la vue de la liste de chat jusqu'au dernier message
            page.update()  # Met à jour la page pour refléter les changements
        
        page.update()
# Appeler la fonction pour initialiser et afficher l'interface principale
    load_main_interface()
